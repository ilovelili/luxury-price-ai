...
.PHONY: help sync db-check migrate dev smoke-analysis smoke-estimate
...
help:
	...
	@echo "  make smoke-analysis   Call local /auction-analysis endpoint"
...
smoke-analysis:
	...
	curl -X POST "http://$(HOST):$(PORT)/auction-analysis" \
	  -H "content-type: application/json" \
	  "$${auth_header[@]}" \
	  -d '{"brand":"CHANEL","category":"バッグ","shape":"ショルダーバッグ","rank":"AB","title":"CHANEL マトラッセ キャビアスキン 黒 ゴールド金具","limit":50}'
